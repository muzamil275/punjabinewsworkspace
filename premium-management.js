(() => {
  ...
  const open = html => { const r=q('#modalRoot'); if (!r) return; r.innerHTML=`<div class="modal workspace-overlay" role="dialog" aria-modal="true">${html}</div>`; r.classList.remove('hidden'); r.setAttribute('aria-hidden','false'); r.querySelector('.close')?.addEventListener('click',close); };
