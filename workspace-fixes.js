(() => {
  ...
  const state = { dates: [], selected: null, loading: false };
  ...
  function pager(){
    ...
    if (!el){
      ...
      el.setAttribute('aria-label','News editions');
  ...
  async function loadEdition(date, announce=true){
    ...
    try{
      ...
      pager();
      if(grid){ grid.innerHTML = (d.posts || []).map(card).join('') || '<div class="loading-card">No stories were published for this edition.</div>'; grid.setAttribute('aria-busy','false'); }
      ...
      if(announce) window.dispatchEvent(new CustomEvent('pnw:news-edition',{detail:{date:state.selected}}));
    }catch(e){
  ...
  async function initEditions(){
    try{
      ...
      const first = saved && state.dates.includes(saved) ? saved : (d.date || state.dates[0]);
  ...
  function openOverlay(kind){
    ...
    root.querySelector('.close').onclick=()=>{root.classList.add('hidden');root.setAttribute('aria-hidden','true');root.innerHTML='';};
  ...
  function bind(){
    document.addEventListener('click', e=>{
      const support=e.target.closest('[data-action="open-support"]');
      if(support){e.preventDefault();openOverlay('support');return;}
      const preview=e.target.closest('#premium-anchor .primary');
      if(preview){e.preventDefault();openOverlay('premium');return;}
    });
